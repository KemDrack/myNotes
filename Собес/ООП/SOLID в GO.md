...
У нас допустим есть како-то сторонний пакет, а мы хотим расширить его функционал, своим методом
Мы создаем собственный тип и встраиваем оригинальный тип, который нам нужен и добавляем функциональность через расширение, а не модификацию 
...
func (b *B) Multiply(a, b int) int { return a * b }

```
Также мы можем **переопределять** типы из **A**, допустим создать метод Add для **B** свой
Нужно стараться добавлять новую функциональность через **расширение** существующего кода, а не через **изменение**
...
Говорит о том, что два типа являются взаимозаменяемыми, если они проявляют такое поведение, при котором вызывающий не может определить разницу

По сути это про интерфейсы. Если у нас есть 2 реализации одного и тоже интерфейса, например **io.Reader**, **io.Writer**. У одного метод **Read**, а у другого **Write**. Мы можем создать свой **кастопный** **тип** у которого будет метод **Read** и можем его подставлять **везде**, где требуется **интерфейс Reader**
...
По сути это тоже про интерфейсы, если нам от всего пакета(os.File), какого нибудь стороннего, у которого два десятка методов, а нам нужен только один.
- Мы делаем интерфейс на своей стороне с этим единственным методом и используем сторонний пакет, как реализацию этого интерфейса
```go
...
У нас не должно быть циклических зависимостей, собственно это и невозможно, потому что Go это не позволяет, будет ошибка

Но если у нас все сделано через интерфейсы, у нас **чистая архитектура**(или Гексагональная архитектура), **слои разделены** друг от друга и зависимость идет **через интерфейсы**, а не напрямую. По сути есть мы следовали всем принципам ранее, то в каком то смысле этот принцип выполяется почти автоматически 
